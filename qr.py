...
# Helper to broadcast message to all registered users
async def broadcast_text_to_all_users(text):
    global tg_app
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "⚠️ **Format galat hai!**\n\n"
            "Aise use karein:\n"
            "`/broadcast Hello users! Ye ek important notice hai.`",
            parse_mode="Markdown"
        )
        return
    ...
    sent_count, failed_count = await broadcast_text_to_all_users(msg_to_send)
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h3>📢 Broadcast Text Message To All Users</h3>
            <form action="/broadcast_text" method="POST">
                <label>Enter Message Text:</label>
                <textarea name="message_text" rows="4" placeholder="Enter message to broadcast to all registered users..." required></textarea>
                <button type="submit" class="btn-blue">📢 Send Message To All Users</button>
            </form>
        </div>
...
@app.route("/broadcast_text", methods=["POST"])
def broadcast_text():
    global bot_loop
    message_text = request.form.get("message_text")
    if message_text:
        future = asyncio.run_coroutine_threadsafe(broadcast_text_to_all_users(message_text), bot_loop)
        sent_count, failed_count = future.result()
        flash(f"✅ Message Broadcasted! Sent: {sent_count}, Failed/Blocked: {failed_count}")
...
def update_settings():
    task_price = request.form.get("task_price")
...
if __name__ == '__main__':
    # 1. Background Thread me Telegram Bot Ko Run Karo
    bot_thread = threading.Thread(target=start_bot)
    bot_thread.daemon = True
    bot_thread.start()

    # 2. Main Thread me Flask Web App Ko Render Ke Dynamic Port Par Run Karo
